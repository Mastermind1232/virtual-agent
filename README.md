...
The module is written and maintained by inGramGames and released on their Patreon: https://www.patreon.com/inGramGames. This repository only repackages each release with a Foundry manifest so it can be installed and updated from Foundry's module manager. Changes from the author's zip: the manifest, download, url and authors fields of `module.json`, and one fix in `scripts/main.js` so apps a GM switched off in Application Access stay off across reloads (the startup migration now respects the GM's saved list).
...
Current packaging: Virtual Agent 1.8.4 (released by inGramGames on 2026-07-16), packaged as 1.8.4.1.
